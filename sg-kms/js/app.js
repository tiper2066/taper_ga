$(document).ready(function(){
    ...
    $('#btn_clock').hover(function() {
        ...
        $('#btn_clock span').css('color', color_login_body);  
    })
    ...
    $('#btn_control').on('click', function() {
        ...
        $("div[id^='alert_']").css('display', 'block');     // 모든 Alert 창 표시
    });
    ...
    /*------- setup control 창 접기 "X" 버튼 ----------*/
    $("#ui_setup .close_setup").on('click', function() {
        $('#ui_setup').toggleClass('on');  // 창 접기
        $("div[id^='alert_']").css('display', 'none');     // 모든 Alert 창 표시 해제
        alertWin_orgStatus() 
    });    
    ...
    $('#wrap').fadeIn(500).css('top','calc(45% - 250px)');

    /*-------------- 장비설정 버튼 클릭 ------------------------*/
    var btn_hwselect = $('#login_box form div').eq(3).children("input");  // 장비설정 버튼
    $(btn_hwselect).on('click', function() {
        $("#login_box").css({"transform":"translateX(-60px)", "opacity":0});
        $("#hwsetup_box").css({"transform":"translateX(-340px)", "opacity":1});
    });
    /*-------------- 장비설정 닫기 버튼 클릭 ------------------------*/
    $("#gologin").on('click', function() {
        $("#login_box").css({"transform":"translateX(0)", "opacity":1});
        $("#hwsetup_box").css({"transform":"translateX(0)", "opacity":0});
    });    
    /*--------------- 장비접속설정 추가 모달 창 열기  ---------------*/
    var btn_hwaccessinfo_add = $("#hwsetup_box #controls form div input:first-child");   // 장비 추가 버튼
    $(btn_hwaccessinfo_add).on('click', function() {
        $("#modal_hwaccessinfo_add").css("z-index",100);  // 레이어 올리고
        $("#modal_hwaccessinfo_add").show();    // 모달 창 보이고..
        $("#modal_hwaccessinfo_add").css({"opacity":1,"transform":"translateY(0)"});  // 서서히 나타나기
    ...
    //var color_placeholder_text = "#8f96a1";      /* placeholder text color 위의것가 같다 둘중하나 제거하자 */    
    ...
    var color_main_gradient = "linear-gradient(to right, #2789f2, #1ba593)";  /* main gradient color */
    ...
    $(main_menu).parent().parent().eq(0).find("a").eq(0).css('color', color_main_blue);   // 대시보드 메뉴 텍스트 선택 상태 
    $(main_menu).parent().parent().eq(0).find("a").eq(0).css('background-color', '#0c101c');     // 대시보드 메뉴 배경 선택 상태 

    // 서브메뉴 - 대시보드 선택
    $(sub_menu).eq(2).find("a").css('color', color_white);    // 대시보드의 서브 첫번째 메뉴 색상 변경
    $(sub_menu).eq(2).find("a").css("background", color_main_gradient);    // 대시보드의 서브 첫번째 메뉴 배경 오버
    $(sub_menu).eq(2).find("a").addClass("on");       //  대시보드 서브 첫번째 메뉴 < 
    ...
    $("#content_02").show();            // 대시보드 > 시스템 상태 컨텐츠 보이기
    $("#content_02").css('opacity',1);        // 대시보드 > 시스템 상태 컨텐츠 보이기
    $("#content_02 .box").css("transform","translateY(0)");   // 대시보드 > 시스템 상태 box 보이기
    ...
    $(main_menu).on('click', function() {
        ...
        if (cmi == 5) {   // 보고서 메뉴 클릭이라면   
            ...
            for (var i = 0; i < mm_num; i++) {
                ...
                $(main_menu).parent().parent().eq(i).find("a").eq(0).css('color', color_login_body);   // 메뉴 텍스트 노말 상태 
                $(main_menu).parent().parent().eq(i).find("a").eq(0).css('background-color', '#20283d');     // 메뉴 배경 노말 상태    
                
                // 서브메뉴 모두 비활성
                $(sub_menu).find("a").css('color', color_login_body);     // 모든 서브 메뉴 색상 노말
            ...
            $(main_menu).parent().parent().eq(cmi).find("a").eq(0).find("img").eq(1).attr('src', '../img/icon_menu_down.png');  // 메뉴 V 선택 상태
            $(main_menu).parent().parent().eq(cmi).find("a").eq(0).css('color', color_main_blue);   // 메뉴 텍스트 오버 상태 
    ...
    $(main_menu).hover(function() {
        var cmi = $(this).parent().parent().index() - 1;      // 현재 선택 객체 index
        $(this).css({'background-color':'#0c101c','color':color_main_blue});
        ...
        if (cmi != mi) {   // 마우스 오버한 메뉴(this) 와 현재 선택 중인 메인메뉴가 다르다면 노말상태로
            $(this).css({'background-color':'#20283d','color':color_login_body});
    ...
    $(sub_menu).on('click', function() {
        ...
        $(this).children().css("background", color_main_gradient);    // 현재 선택 서브메뉴 배경 오버
        ...
        $(window).trigger('resize');  // 그래프 강제 사이즈 조정

    });
    //--------------- 서브메뉴 마우스 오버 이벤트 --------------
    $(sub_menu).hover(function() {      
        $(this).find("a").css('color', color_white);    
    }, function() {
        var cmi = $(this).parent().parent().parent().index() - 1;     // 현재 선택된 메인메뉴 index
        var smi_crr = $(this).index();     // 현재 선택된 메인메뉴 index  

        if (smi_crr != smi) {
            $(this).find("a").css('color', color_login_body); 
        } else if (cmi != mi) {
            $(this).find("a").css('color', color_login_body); 
        } else {
            return;
    ...
    /*==========================================================*/
    /*      테이블 row 확장 UI  -- Edit icon 클릭 : 수정 시     */
    /*==========================================================*/    
    var btn_edit = $("#content_10 #damoagent_table > table > tbody > tr:nth-child(3) .btn_editicon");   // edit icon selector
    var btn_confirm = $("#content_10 #damoagent_table > table > tbody > tr:nth-child(4) .btn_confirm");   // ok icon selector
    var btn_cancel = $("#content_10 #damoagent_table > table > tbody > tr:nth-child(4) .btn_cancel");   // cancel icon selector
    
    $(btn_edit).on('click', function() {     // row 수정 아이콘 클릭
        reverse = false;
        $("#content_10 #damoagent_table > table > tbody > tr:nth-child(3)").hide();     //  선택한 row 숨김
        $("#content_10 #damoagent_table > table > tbody > tr:nth-child(4)").show();     //  확장된 수정 row 보임
    });
    $(btn_confirm).on('click', function() {  // row 수정 완료 아이콘 클릭
        reverse = true;
        $("#content_10 #damoagent_table > table > tbody > tr:nth-child(4)").hide();     //  수정된 row 숨김
        $("#content_10 #damoagent_table > table > tbody > tr:nth-child(3)").show();     //  기존 row 보임
    });
    $(btn_cancel).on('click', function() {  // row 수정 완료 아이콘 클릭
        reverse = true;
        $("#content_10 #damoagent_table > table > tbody > tr:nth-child(4)").hide();     //  수정된 row 숨김
        $("#content_10 #damoagent_table > table > tbody > tr:nth-child(3)").show();     //  기존 row 보임
    });
    ...
    $("table tbody tr").on('click', function() {
        if (reverse === true) {      // 일번적인 table 일경우 반전하고
            $(this).parent().children().children().css({'background-color':'transparent', 'color':color_basic_text});    // 모든 td 배경을 제거한다.  
            ...
            $(this).children().css({'background-color':'#283048', 'color':color_white});   // 현재 선택한 tr > td의 색상을 바꿔줌, tr hover 색상은 아래 적용 중    
        } else {      // 예외 테이블일 경우 
            return true;  // 다른 table 을 위해서 true 로 변경
        }
    });
    //--------------- 테이블 tr 마우스 오버  --------------
    ...
    /*--------------- 테이블 설정 버튼에서 -- 접근 허용 기간 설정 모달 2 창 열기  ---------------*/
    var btn_datetime = $("#damoagent_table > table > tbody > tr.edit_expand > td:nth-child(6) > input.btn_gray_sm_dot.btn_s.w40");   // 접근허용기간 설정 버튼 
    $(btn_datetime).on('click', function() {
        $("#modal2_access_permit_time").css("z-index",100);  // 레이어 올리고
        $("#modal2_access_permit_time").show();    // 모달 창 보이고..
        $("#modal2_access_permit_time").css({"opacity":1,"transform":"translateY(0)"});  // 서서히 나타나기
        $("#overlay").show().css('z-index', 99);
        depth = '2-1';   // 2depth 모달이 1depth 로 열린 창 알림
    });    
    ...
    function closeModal() {
        ...
        } else {    // 2-1 등 기타 상황으로 2depth 창이 열릴때
            $("div[id^='modal2_']").css({"opacity":0,"transform":"translateY(100px)"});  // 모든 모달 창 투명하게 원래위치로 
            setTimeout(() => {
                $("div[id^='modal2_']").css("z-index",0);
                $("div[id^='modal2_']").hide();  // 모든 모달 창 숨기고..
    ...
    function diskChart() {
        ...
        var options = {
          ...
          pieHole: 0.3,
          colors: [ color_green, color_main_blue ],
    ...
    function memoryChart() {
      ...
      var options = {
        ...
        pieHole: 0.3,
        colors: [ color_green, color_main_blue, color_login_body ],
        chartArea: { left: 0, top: 0, width:'90%', height:'90%' }
      };
    ...
    /*----------------------- 시스템 상태 : 최근 1분 CPU 사용률 라인 차트 start -----------------------*/
    function drawBasic() {
        ...
        var options = {
            ...
            colors: [ color_main_blue ]
        };

        var chart = new google.visualization.LineChart(document.getElementById('lineChart0_1'));

        chart.draw(data, options);
    }

    /*----------------------- 대시보드(통합) : CPU 사용률 현황 차트 -----------------------*/
    function drawBasic2() {
    
        var data2 = new google.visualization.DataTable();
        data2.addColumn('number', 'X');
        data2.addColumn('number', 'KMS1');
    ...
    /*----------------------- 대시보드(통합) : SAP 암호화 현황 차트 -----------------------*/
    function drawBasic4() {
    
        var data4 = new google.visualization.DataTable();
        data4.addColumn('number', 'X');
        data4.addColumn('number', 'KMS1');
    ...
    /*----------------------- 대시보드(통합) : SAP 복호화 현황 차트 -----------------------*/
    function drawBasic5() {
    
        var data5 = new google.visualization.DataTable();
        data5.addColumn('number', 'X');
        data5.addColumn('number', 'KMS1');
    ...
    /*                          바 차트                         */
    /*==========================================================*/  
    /*----------------------- 대시보드(통합) : 자원사용율 현황 바 차트 (디스크 영역) -----------------------*/
    google.charts.load('current', {'packages':['bar']});
    google.charts.setOnLoadCallback(drawChart1);

    function drawChart1() {
      var data1 = google.visualization.arrayToDataTable([
        ['KMS', 'Free', 'General', 'Cloud'],
        ['2014', 100, 40, 20],
        ['2015', 117, 46, 25],
        ['2016', 66, 112, 30],
        ['2017', 103, 54, 35]
      ...
      var options1 = {
        ...
        bars: 'vertical',
        bar: { groupWidth: '30%' },     // 컬럼그래프 너비 설정
        vAxis: { format: 'percent' },
        width: 600,
        height: 200,
        legend: { position: 'right', alignment: 'start' },
        //colors: ['#1b9e77', '#d95f02', '#7570b3'],
        colors: [ color_main_blue, 'green', color_login_body ],
        isStacked: true
      };
    ...
    bt_backupSetupToggle.click(function() {
        ...
        if (t1_on) {    // 활성화 
            $("#content_60 .cbox1 .toggle_txt").css({"color":color_main_blue, "font-weight":"500"});   // 텍스트 컬러를 파란색으로 
    ...
    bt_drSyncSetupToggle.click(function() {
        ...
        if (t2_on) {    // 활성화 
            $("#content_60 .cbox2 .toggle_txt").css({"color":color_main_blue, "font-weight":"500"});   // 텍스트 컬러를 파란색으로 
    ...
    $("#btn_itemSelect").on('click', function() {
        if (itemSelect) {    // 활성화 상태면
            $("#modal_alim_add #btn_switch .toggle_txt").css({"color":color_main_blue, "font-weight":"500"});   // on/off 텍스트 컬러를 파란색으로 
    ...
    //============= 보고서 설정 활성화 토글 버튼 클릭  ================//
    var reportSelect = false;    // 체크 준비
    $("#btn_report_on").on('click', function() {
        var disArea = $("#content_50 > div.box.cbox1 > div.setupArea1 > form > ul > li");  // li 까지 선택자
        console.log('click');
        if (reportSelect) {    // 비활성화 상태로
            $("#content_50 #btn_switch .toggle_txt").css({"color":color_main_blue, "font-weight":"500"});   // on/off 텍스트 컬러를 파란색으로 
            $("#content_50 #btn_switch p").eq(0).css("display", "none");   // off 
            $("#content_50 #btn_switch p").eq(1).css("display", "inline-block");   // on
            $(disArea).eq(3).children("input").attr("disabled",false);     // 보고서 제목
            $(disArea).eq(4).children("input").attr("disabled",false);     // 보고서 작성자
            $(disArea).eq(5).children("input").attr("disabled",false);     // 보고서 수신자
            $("#content_50 .setupArea1 .select_big select").attr("disabled",false);     // 모든 select 콘트롤 
            $(disArea).eq(7).children("input").attr("disabled",false);     // 작성기간 부터
            $(disArea).eq(8).children("input").attr("disabled",false);     // 작성기간 까지
            $("#content_50 .setupArea1 ul li > input[type='checkbox']").attr("disabled",false);     // 모든 checkbox 콘트롤 
        } else {    // 활성화 상태로 
            $("#content_50 #btn_switch .toggle_txt").css({"color":"#808080", "font-weight":"300"});   // on/off 텍스트 컬러를 회색으로  
            $("#content_50 #btn_switch p").eq(0).css("display", "inline-block");   // off 
            $("#content_50 #btn_switch p").eq(1).css("display", "none");   // on
            $(disArea).eq(3).children("input").attr("disabled",true);     // 보고서 제목
            $(disArea).eq(4).children("input").attr("disabled",true);     // 보고서 작성자
            $(disArea).eq(5).children("input").attr("disabled",true);     // 보고서 수신자
            $("#content_50 .setupArea1 .select_big select").attr("disabled",true);     // 모든 select 콘트롤 
            $(disArea).eq(7).children("input").attr("disabled",true);     // 작성기간 부터
            $(disArea).eq(8).children("input").attr("disabled",true);     // 작성기간 까지
            $("#content_50 .setupArea1 ul li > input[type='checkbox']").attr("disabled",true);     // 모든 checkbox 콘트롤 
        } 
    ...
    /*               대시보드 - 보기 체크버튼 이벤트            */
    /*==========================================================*/     
    var checkLable01 = $("#content_02 > div.view_control > div.selectview > label:nth-child(3)");  // 전체 글자
    var checkLable02 = $("#content_02 > div.view_control > div.selectview > label:nth-child(5)");  // 일반 환경 글자
    var checkLable03 = $("#content_02 > div.view_control > div.selectview > label:nth-child(7)");  // 클라우드 환경 글자

    $("#totalEnv").on('click', function() {    // 전체 보기 버튼 클릭 이벤트
        $("#content_02 > h1 > span").text('- 전체'); 
        $("#content_02 > #pageheader > p.desc").text("일반 환경과 클라우드 환경 모두의 장비 시스템 상태를 볼 수 있습니다.");
        $("#pageheader > p.path > span:nth-child(2)").text("전체");
        $(checkLable01).css({"color":color_main_blue, "font-weight":"400"});   // 레이블 폰트 컬러 굵기 변경
        $(checkLable02).css({"color":color_basic_text, "font-weight":"300"});   // 레이블 폰트 컬러 굵기 변경
        $(checkLable03).css({"color":color_basic_text, "font-weight":"300"});   // 레이블 폰트 컬러 굵기 변경

    });
    $("#normalEnv").on('click', function() {    // 일반 환경 보기 버튼 클릭 이벤트
        $("#content_02 > h1 > span").text('- 일반 환경');
        $("#content_02 > #pageheader > p.desc").text("일반 환경의 장비 시스템 상태를 볼 수 있습니다.");
        $("#pageheader > p.path > span:nth-child(2)").text("일반 환경");
        $(checkLable01).css({"color":color_basic_text, "font-weight":"300"});   // 레이블 폰트 컬러 굵기 변경
        $(checkLable02).css({"color":color_main_blue, "font-weight":"400"});   // 레이블 폰트 컬러 굵기 변경
        $(checkLable03).css({"color":color_basic_text, "font-weight":"300"});   // 레이블 폰트 컬러 굵기 변경
    });    
    $("#cloudEnv").on('click', function() {    // 클라우드 환경 보기 버튼 클릭 이벤트
        $("#content_02 > h1 > span").text('- 클라우드 환경');
        $("#content_02 > #pageheader > p.desc").text("클라우드 환경의 장비 시스템 상태를 볼 수 있습니다.");
        $("#pageheader > p.path > span:nth-child(2)").text("클라우드 환경");
        $(checkLable01).css({"color":color_basic_text, "font-weight":"300"});   // 레이블 폰트 컬러 굵기 변경
        $(checkLable02).css({"color":color_basic_text, "font-weight":"300"});   // 레이블 폰트 컬러 굵기 변경
        $(checkLable03).css({"color":color_main_blue, "font-weight":"400"});   // 레이블 폰트 컬러 굵기 변경
    });

    /*=====================================================================*/
    /*   프로세스 상태 현황 KMS1 서버 클릭이벤트(개별 시스템보기로 이동)   */
    /*=====================================================================*/

    //============= 개별 시스템 보기로 가는 함수   ================//
    function individual_view(smNum) {
        // 메인 메뉴 모두 비활성        
        for (var i = 0; i < mm_num; i++) {
            $(main_menu).parent().parent().eq(i).find("a").eq(0).find("img").eq(0).attr('src', mImgArray[i]);  // 메뉴 아이콘 노말 상태
            $(main_menu).parent().parent().eq(i).find("a").eq(0).css('color', color_login_body);   // 메뉴 텍스트 노말 상태 
        ...
        // 선택 메인 메뉴 활성화 
        var cmi = 0;     // 현재 선택된 메인메뉴 index
        smi = smNum;     // 현재 선택된 서브메뉴 index
        mi = cmi    // 현재 노드의 인덱스 저장 (부모인 ul의 index)
        ...
        // 선택 서브메뉴 활성화
        $(sub_menu).eq(smi).children().css('color', color_white);    // 현재 서브 메뉴 색상 오버
        $(sub_menu).eq(smi).children().css("background", color_main_gradient);    // 현재 선택 서브메뉴 배경 오버
        $(sub_menu).eq(smi).children().addClass("on");       //  대시보드 서브 첫번째 메뉴 < 
    ...
    $("#process_table > table > tbody > tr:nth-child(1)").on('click', function() {   // 개별 서버 클릭 시 
        individual_view(0);
    });

    //============= 대시 보드로 돌아가기   ================//
    var backBtn_dashboad = $("#content_00 > div.view_control > div.rpt > input");
    $(backBtn_dashboad).on('click', function() {
        individual_view(2);
    });
